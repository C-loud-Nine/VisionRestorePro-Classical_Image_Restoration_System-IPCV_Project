...
Version: 4.9 - Custom BM3D Implementation
"""
...
# Custom BM3D Implementation - Set to True since we have our own implementation
HAS_BM3D = True
...
# =============================================================================
# CUSTOM BM3D IMPLEMENTATION
# =============================================================================

def compute_dct_basis(size: int) -> np.ndarray:
    """
    Compute DCT basis matrices for given size.
    """
    ...
    return basis * (2.0 / size)

def block_matching(img: np.ndarray, ref_patch: np.ndarray, ref_pos: Tuple[int, int],
                  search_window: int, max_matches: int, patch_size: int, 
                  sigma: float) -> List[Tuple[int, int]]:
    """
    Find similar patches using block matching with adaptive threshold.
    """
    i, j = ref_pos
    h, w = img.shape
    ...
    i_max = min(h - patch_size, i + search_radius)
    j_min = max(0, j - search_radius)
    j_max = min(w - patch_size, j + search_radius)
    
    # Calculate reference patch statistics
    ref_mean = np.mean(ref_patch)
    ref_std = np.std(ref_patch) + 1e-8
    
    # Adaptive threshold based on noise level and patch similarity
    distance_threshold = 2.0 * sigma + 0.1 * ref_std
    
    similar_patches = [(i, j)]  # Always include reference patch
    
    for pi in range(i_min, i_max + 1, 2):  # Step of 2 for efficiency
        for pj in range(j_min, j_max + 1, 2):
            if pi == i and pj == j:
                continue
                
            candidate_patch = img[pi:pi+patch_size, pj:pj+patch_size]
            
            # Calculate normalized cross-correlation for similarity
            cand_mean = np.mean(candidate_patch)
            cand_std = np.std(candidate_patch) + 1e-8
            
            # Zero-mean normalized cross-correlation
            normalized_ref = (ref_patch - ref_mean) / ref_std
            normalized_cand = (candidate_patch - cand_mean) / cand_std
            
            # Calculate similarity (1 - NCC since we want distance)
            similarity = 1.0 - np.mean(normalized_ref * normalized_cand)
            
            if similarity < distance_threshold and len(similar_patches) < max_matches:
                similar_patches.append((pi, pj))
    
    return similar_patches

def dct_2d(patch: np.ndarray, dct_basis: np.ndarray) -> np.ndarray:
    """
    Apply 2D DCT using precomputed basis.
    """
    size = patch.shape[0]
    ...
    for u in range(size):
        for v in range(size):
            result[u, v] = np.sum(patch * dct_basis[u, v, :, :])
    
    return result

def idct_2d(transformed_patch: np.ndarray, dct_basis: np.ndarray) -> np.ndarray:
    """
    Apply inverse 2D DCT using precomputed basis.
    """
    size = transformed_patch.shape[0]
    result = np.zeros_like(transformed_patch)
    
    for x in range(size):
        for y in range(size):
            result[x, y] = np.sum(transformed_patch * dct_basis[:, :, x, y])
    
    return result

def apply_3d_transform(group_3d: np.ndarray, dct_basis: np.ndarray) -> np.ndarray:
    """
    Apply 3D transform: 2D DCT on each patch + 1D DCT along third dimension.
    """
    patch_size = group_3d.shape[0]
    group_size = group_3d.shape[2]
    
    # Apply 2D DCT to each patch in the group
    dct_2d_group = np.zeros_like(group_3d)
    for k in range(group_size):
        dct_2d_group[:, :, k] = dct_2d(group_3d[:, :, k], dct_basis)
    
    # Apply 1D DCT along the third dimension
    dct_3d_group = np.zeros_like(dct_2d_group)
    for i in range(patch_size):
        for j in range(patch_size):
            dct_3d_group[i, j, :] = dct(dct_2d_group[i, j, :], norm='ortho')
    
    return dct_3d_group

def apply_inverse_3d_transform(transformed_group: np.ndarray, dct_basis: np.ndarray) -> np.ndarray:
    """
    Apply inverse 3D transform.
    """
    patch_size = transformed_group.shape[0]
    group_size = transformed_group.shape[2]
    
    # Inverse 1D DCT along third dimension
    idct_2d_group = np.zeros_like(transformed_group)
    for i in range(patch_size):
        for j in range(patch_size):
            idct_2d_group[i, j, :] = idct(transformed_group[i, j, :], norm='ortho')
    
    # Inverse 2D DCT for each patch
    reconstructed_group = np.zeros_like(idct_2d_group)
    for k in range(group_size):
        reconstructed_group[:, :, k] = idct_2d(idct_2d_group[:, :, k], dct_basis)
    
    return reconstructed_group

def hard_thresholding(coefficients: np.ndarray, threshold: float) -> np.ndarray:
    """
    Apply hard thresholding to transform coefficients.
    """
    thresholded = coefficients.copy()
    thresholded[np.abs(coefficients) < threshold] = 0
    return thresholded

def collaborative_filtering_3d(patches_positions: List[Tuple[int, int]], 
                              img: np.ndarray, sigma: float,
                              hard_threshold: float, 
                              dct_basis: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
    Apply collaborative filtering in 3D transform domain.
    """
    patch_size = dct_basis.shape[0]
    group_3d = []
    
    # Extract patches and form 3D group
    for i, j in patches_positions:
        patch = img[i:i+patch_size, j:j+patch_size]
        group_3d.append(patch)
    
    group_3d = np.stack(group_3d, axis=2)  # Shape: (patch_size, patch_size, group_size)
    
    # Apply 3D transform (2D DCT + 1D DCT)
    transformed_group = apply_3d_transform(group_3d, dct_basis)
    
    # Hard thresholding
    threshold = hard_threshold * sigma
    transformed_group_thresh = hard_thresholding(transformed_group, threshold)
    
    # Inverse 3D transform
    denoised_group = apply_inverse_3d_transform(transformed_group_thresh, dct_basis)
    
    # Calculate weights based on number of non-zero coefficients
    non_zero_coeffs = np.sum(np.abs(transformed_group_thresh) > 1e-6)
    total_coeffs = transformed_group_thresh.size
    
    if non_zero_coeffs == 0:
        weight = 1.0 / (sigma ** 2)  # Fallback weight
    else:
        weight = 1.0 / (sigma ** 2 * non_zero_coeffs)
    
    weights = np.full(len(patches_positions) - 1, weight)  # -1 because reference is first
    
    # Return denoised patches (excluding reference)
    denoised_patches = []
    for idx in range(1, len(patches_positions)):  # Skip reference
        denoised_patches.append(denoised_group[:, :, idx])
    
    return np.array(denoised_patches), weights

def apply_2d_dct_denoising(patch: np.ndarray, sigma: float, dct_basis: np.ndarray) -> np.ndarray:
    """
    Fallback 2D DCT denoising when not enough similar patches are found.
    """
    # Apply 2D DCT
    dct_coeffs = dct_2d(patch, dct_basis)
    
    # Hard thresholding
    threshold = 2.0 * sigma
    dct_coeffs_thresh = hard_thresholding(dct_coeffs, threshold)
    
    # Inverse 2D DCT
    denoised_patch = idct_2d(dct_coeffs_thresh, dct_basis)
    
    return denoised_patch

def handle_image_edges(img: np.ndarray, denoised: np.ndarray, weight_accum: np.ndarray,
                      patch_size: int, sigma: float, dct_basis: np.ndarray) -> np.ndarray:
    """
    Handle edge regions that weren't processed by the main loop.
    """
    h, w = img.shape
    ...
    for i in range(0, h - patch_size + 1, patch_size):
        ...
        if j >= 0:
            patch = img[i:i+patch_size, j:j+patch_size]
            denoised_patch = apply_2d_dct_denoising(patch, sigma, dct_basis)
            denoised[i:i+patch_size, j:j+patch_size] += denoised_patch
            weight_accum[i:i+patch_size, j:j+patch_size] += 1.0
...
def bm3d_denoising(img: np.ndarray, sigma: float, 
                  patch_size: int = 8, 
                  step_size: int = 3,
                  search_window: int = 39,
                  max_matches: int = 16,
                  hard_threshold: float = 2.7,
                  window_function: str = 'Hamming') -> np.ndarray:
    """
    BM3D denoising algorithm implementation from scratch.
    
    Parameters:
    -----------
    img : np.ndarray
        Input noisy image (grayscale, 0-1 range)
    sigma : float
        Noise standard deviation
    patch_size : int
        Size of patches (typically 8)
    step_size : int
        Step size for patch extraction
    search_window : int
        Search window size for block matching
    max_matches : int
        Maximum number of similar patches per group
    hard_threshold : float
        Hard threshold coefficient for 3D transform
    window_function : str
        Window function for weighting ('Hamming', 'Unity')
    ...
    weight_accum = np.zeros_like(img)
    
    # Create window function for patch weighting
    if window_function == 'Hamming':
        window_1d = np.hamming(patch_size)
        window_2d = np.outer(window_1d, window_1d)
    else:  # Unity
        window_2d = np.ones((patch_size, patch_size))
    
    # Precompute DCT basis for faster computation
    ...
    for i in range(0, h - patch_size + 1, step_size):
        for j in range(0, w - patch_size + 1, step_size):
            # Reference patch
            ref_patch = img[i:i+patch_size, j:j+patch_size]
            
            # Find similar patches using block matching
            similar_patches = block_matching(img, ref_patch, (i, j), 
                                           search_window, max_matches, 
                                           patch_size, sigma)
            
            if len(similar_patches) < 2:
                # Not enough similar patches, use basic filtering
                denoised_patch = apply_2d_dct_denoising(ref_patch, sigma, dct_basis)
                denoised[i:i+patch_size, j:j+patch_size] += denoised_patch * window_2d
                weight_accum[i:i+patch_size, j:j+patch_size] += window_2d
                continue
            
            # Form 3D group and apply collaborative filtering
            denoised_group, group_weights = collaborative_filtering_3d(
                similar_patches, img, sigma, hard_threshold, dct_basis
            )
            
            # Aggregate results back to image
            for idx, (pi, pj) in enumerate(similar_patches[1:]):  # Skip reference patch
                patch_weight = group_weights[idx] * window_2d
                denoised[pi:pi+patch_size, pj:pj+patch_size] += denoised_group[idx] * patch_weight
                weight_accum[pi:pi+patch_size, pj:pj+patch_size] += patch_weight
    
    # Handle edge cases and normalize
    denoised = handle_image_edges(img, denoised, weight_accum, patch_size, sigma, dct_basis)
    
    # Avoid division by zero
    weight_accum[weight_accum == 0] = 1
    denoised = denoised / weight_accum
    
    return np.clip(denoised, 0, 1)

def bm3d_rgb_denoising(img_rgb: np.ndarray, noise_level: float) -> np.ndarray:
    """
    BM3D denoising for RGB images using our custom implementation.
    Applies denoising to luminance channel only for efficiency.
    """
    # Convert to YCbCr
    img_u8 = (img_rgb * 255).astype('uint8')
    ycrcb = cv2.cvtColor(img_u8, cv2.COLOR_RGB2YCrCb).astype('float32') / 255.0
    
    # Denoise Y channel only using our custom BM3D
    y_channel = ycrcb[..., 0]
    
    # Convert noise level to appropriate scale
    sigma_psd = noise_level * 0.1
    
    try:
        denoised_y = bm3d_denoising(y_channel, sigma_psd)
        ...
        warnings.warn(f"Custom BM3D failed: {e}, falling back to wavelet denoising")
...
def denoise_luminance(img_rgb: np.ndarray, sigma: float, use_bm3d: bool = True):
    ...
    if use_bm3d:
        try:
            # Use our custom BM3D implementation
            den_y = bm3d_denoising(y, sigma/255.0)
            den_y = np.clip(den_y, 0, 1)
        except Exception as e:
            print(f"Custom BM3D failed in luminance: {e}")
...
def adaptive_denoising(img_rgb: np.ndarray, degradation_info: Dict[str, Any], params: Dict[str, Any]) -> np.ndarray:
    ...
    if degradation_info['salt_pepper_present']:
        ...
        # Use our custom BM3D or wavelet for Gaussian noise
        if params['USE_BM3D']:
            try:
                # Use our custom BM3D implementation
                denoised = bm3d_rgb_denoising(img_rgb, degradation_info['noise_level'])
            except Exception as e:
                print(f"Custom BM3D failed: {e}, falling back to wavelet")
...
def main():
    ...
    with col2:
        ...
        st.markdown('<p class="app-subtitle">Intelligent Image Restoration with Custom BM3D Implementation</p>', unsafe_allow_html=True)
    ...
    use_bm3d = st.sidebar.checkbox("Use Custom BM3D Denoising", True,
                                  help="Use our custom BM3D implementation for superior denoising")
    st.sidebar.markdown('</div>', unsafe_allow_html=True)
    ...
    # Display BM3D status
    if use_bm3d:
        st.sidebar.markdown('<div class="status-indicator status-success">✅ Custom BM3D Enabled</div>', unsafe_allow_html=True)
    else:
    ...
    with tab5:
        ...
        **VisionRestore Pro** is a state-of-the-art application for restoring images using advanced computational photography techniques.
        ...
        - **🔧 Custom BM3D**: Our own implementation of BM3D denoising algorithm
        
        #### 🛠️ Technical Highlights:
        
        - **Custom BM3D Implementation**: Full BM3D algorithm from scratch
        ...
        with col1:
            st.metric("Custom BM3D", "✅ Implemented")
        with col2:
